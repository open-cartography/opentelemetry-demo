...
func main() {
	...
	f, err := os.Open("./NecefliMasrapa.png")
	if err != nil {
		log.Fatal(err)
	}
	necefliImage, err := png.Decode(f)
	if err != nil {
		log.Fatal(err)
	}
	NecefliMasrapa := story{
		chapter{
			title:    "The NecefliMasrapa",
			duration: 5 * time.Minute,
			sources: []source{
				{
					image: necefliImage,
	...
	go tellStory(NecefliMasrapa)

	http.Handle("/metrics", promhttp.HandlerFor(reg, promhttp.HandlerOpts{}))
	log.Println("Serving NecefliMasrapa metrics, SIGTERM to abort…")
